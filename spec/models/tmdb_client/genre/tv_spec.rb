...
RSpec.describe TmdbClient::Genre::Tv, type: :model do

  describe "CLASS METHODS" do

    it ".url" do
      expect(described_class.url).to eq("https://api.themoviedb.org/3/genre/tv/list")
    end
    ...
    it ".path" do
      expect(described_class.path).to eq("genre/tv/list")
    end
    ...
    describe "When authenticated with API KEY" do
      ...
      context ".all" do
        let(:expected_response) { [{"id"=>10759, "name"=>"Action & Adventure"}, {"id"=>16, "name"=>"Animation"}] }

        it "returns with the list of genres" do
          VCR.use_cassette("tv_genres_api_key") do
    ...
    describe "When authenticated with Bearer Token" do
      ...
      context ".all" do
        ...
        it "returns with the list of genres" do
          VCR.use_cassette("tv_genres_bearer") do
            expect(described_class.all).to eq(expected_response)
          end
        end
      end
